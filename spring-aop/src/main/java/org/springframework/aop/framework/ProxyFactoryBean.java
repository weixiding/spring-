...
/**
 			说破大天他也就是个bean，只不过是工厂bean，getbean("他")会返回getobject方法的bean
 			此处继承了BeanClassLoaderAware, BeanFactoryAware接口，则beanfactory 会在bean实例化后进行初始化设置相关参数
 */
...
public class ProxyFactoryBean extends ProxyCreatorSupport
	...
	public Object getObject() throws BeansException {
		...
		//对singleton类型的bean 和prototype类型的bean进行区分，以生成不同的代理
		if (isSingleton()) {
	...
	private synchronized void initializeAdvisorChain() throws AopConfigException, BeansException {
		...
		//进行初始化  this.interceptorNames是我们配置的
		if (!ObjectUtils.isEmpty(this.interceptorNames)) {
			//一些相关的判断条件
