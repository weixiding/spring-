...
public abstract class WebContentGenerator extends WebApplicationObjectSupport {
	...
	/**
	 * 由构造函数可以restrictDefaultSupportedMethods来决定是否进行支持的方法检查
	 */
	...
	protected final void checkAndPrepare(
		...
		// 如果 supportedMethods不为空的话，就检查是否支持该类型的请求，如果为空就不检查
		...
		// 如果requireSession 为true 且session 不存在则抛出异常，但是requireSession默认为flase
		...
		// 给response 设置缓存过期时期,就是通过http请求头设置过期时期
